...
for ( $day = 1; $day <= $daysinmonth; ++$day ) {
	...
	$isTherePost = array_key_exists($day, $daywithpost);
	$cellStyle = $isTherePost ? 'background-color: #f7f0f6;' : '';

	if ( $day == gmdate('j', current_time('timestamp')) && $thismonth == gmdate('m', current_time('timestamp')) && $thisyear == gmdate('Y', current_time('timestamp')) )
		$calendar_output .= '<td id="today" style="'.$cellStyle.'">';
	else
		$calendar_output .= '<td style="'.$cellStyle.'">';

	if ( $isTherePost ) // any posts today?
		$calendar_output .= '<a data-tooltip data-options="disable_for_touch:true" class="has-tip" href="' . ( sizeof( $daywithpost[ $day ] ) > 1 ? add_query_arg( array( 'post_type' => 'sp_event' ), get_day_link( $thisyear, $thismonth, $day ) ) . '" title="' . sprintf( '%s événements', ( sizeof( $daywithpost[ $day ] ) ) ) : get_post_permalink( $daywithpost[ $day ][0], false, true ) . '" title="' . esc_attr( $ak_titles_for_day[ $day ] ) ) . "\">$day</a>";
	else
		$calendar_output .= $day;
	$calendar_output .= '</td>';
...
if ( $id && $show_all_events_link )
	$calendar_output .= '<div class="sp-calendar-link sp-view-all-link"><a href="' . get_permalink( $id ) . '">' . __( 'Voir tous les événements', 'sportspress' ) . '</a></div>';
?>
